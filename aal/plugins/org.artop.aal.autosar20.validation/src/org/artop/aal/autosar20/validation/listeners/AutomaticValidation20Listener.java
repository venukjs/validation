...
import autosar20.util.Autosar20ResourceImpl;

public class AutomaticValidation20Listener extends AbstractAutomaticValidationListener {
	...
	protected boolean isTargetResource(Resource resource) {
		...
		return resource instanceof Autosar20ResourceImpl ? true : false;
	}
	...
	protected boolean isShortNameChange(ResourceSetChangeEvent event) {
		...
		if (notifications.size() == 1) {
			...
			if (notification instanceof ENotificationImpl && notification.getNotifier() instanceof EObject) {
				EClassifier ownerType = InfrastructurePackage.eINSTANCE.getIdentifiable();
				return ownerType.isInstance(notification.getNotifier())
						&& notification.getFeatureID(ownerType.getInstanceClass()) == InfrastructurePackage.IDENTIFIABLE__SHORT_NAME;
			}
		}
