...
public class CSVWriter {
	...
	public CSVWriter(String fileName, char delimiter, Charset charset) {
		if (fileName == null) {
			throw new IllegalArgumentException("Parameter fileName can not be null."); //$NON-NLS-1$
		}

		if (charset == null) {
			throw new IllegalArgumentException("Parameter charset can not be null."); //$NON-NLS-1$
		}
	...
	public CSVWriter(String fileName) {
		this(fileName, Letters.COMMA, Charset.forName("ISO-8859-1")); //$NON-NLS-1$
	}
	...
	public CSVWriter(Writer outputStream, char delimiter) {
		if (outputStream == null) {
			throw new IllegalArgumentException("Parameter outputStream can not be null."); //$NON-NLS-1$
		}
	...
	public void write(String content, boolean preserveSpaces) throws IOException {
		...
		if (!textQualify
				...
				&& (content.indexOf(userSettings.TextQualifier) > -1 || content.indexOf(userSettings.Delimiter) > -1 || !useCustomRecordDelimiter
						&& (content.indexOf(Letters.LF) > -1 || content.indexOf(Letters.CR) > -1) || useCustomRecordDelimiter
						&& content.indexOf(userSettings.RecordDelimiter) > -1 || firstColumn && content.length() > 0
						&& content.charAt(0) == userSettings.Comment || firstColumn && content.length() == 0)) {
			textQualify = true;
		}
		...
		if (textQualify) {
			...
			if (userSettings.EscapeMode == ESCAPE_MODE_BACKSLASH) {
				content = replace(content, EMPTY_STRING + Letters.BACKSLASH, EMPTY_STRING + Letters.BACKSLASH + Letters.BACKSLASH);
				content = replace(content, EMPTY_STRING + userSettings.TextQualifier, EMPTY_STRING + Letters.BACKSLASH + userSettings.TextQualifier);
			} else {
				content = replace(content, EMPTY_STRING + userSettings.TextQualifier, EMPTY_STRING + userSettings.TextQualifier
						+ userSettings.TextQualifier);
			}
		} else if (userSettings.EscapeMode == ESCAPE_MODE_BACKSLASH) {
			content = replace(content, EMPTY_STRING + Letters.BACKSLASH, EMPTY_STRING + Letters.BACKSLASH + Letters.BACKSLASH);
			content = replace(content, EMPTY_STRING + userSettings.Delimiter, EMPTY_STRING + Letters.BACKSLASH + userSettings.Delimiter);

			if (useCustomRecordDelimiter) {
				content = replace(content, EMPTY_STRING + userSettings.RecordDelimiter, EMPTY_STRING + Letters.BACKSLASH
						+ userSettings.RecordDelimiter);
			} else {
				content = replace(content, EMPTY_STRING + Letters.CR, EMPTY_STRING + Letters.BACKSLASH + Letters.CR);
				content = replace(content, EMPTY_STRING + Letters.LF, EMPTY_STRING + Letters.BACKSLASH + Letters.LF);
			}

			if (firstColumn && content.length() > 0 && content.charAt(0) == userSettings.Comment) {
				if (content.length() > 1) {
					content = EMPTY_STRING + Letters.BACKSLASH + userSettings.Comment + content.substring(1);
				} else {
					content = EMPTY_STRING + Letters.BACKSLASH + userSettings.Comment;
				}
			}
		}
	...
	private void checkClosed() throws IOException {
		if (closed) {
			throw new IOException("This instance of the CsvWriter class has already been closed."); //$NON-NLS-1$
		}
	}
